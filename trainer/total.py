...
import argparse
from MoboGaze import MoboGaze

def main(config, m_conf):
    ...
    model_config = edict(yaml.load(open(m_conf), Loader=yaml.FullLoader))
    net = MoboGaze(model_config)
    ...
    ckpt = torch.load(pretrain.path)
    #ckpt.pop('classifier.1.weight')
    #ckpt.pop('classifier.1.bias')
    print(net)
    net.load_state_dict(
                ckpt
                , strict=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", '--config', type=str, default=None, help="Configuration file"
    )
